    ...
    for (let i = 0; i < file.length; i++) {
      ...
      if (validImageTypes.includes(fileType)) {
        ...
        const data = await fetch(
          "https://test.warpbay.com/api/v1/user/upload_picture",
          {
            method: "POST",
            headers: {
              Authorization:
  ...
  return (
    <>
      <div className="flex justify-center w-full items-center ">
        <div className="rounded-lg border w-full border-gray-300 bg-gray-50">
          <div className="m-4">
              ...
              {files.map((file, key) => {
                return (
                  <div key={key} className="overflow-hidden relative">
                    <div
                      className="cursor-pointer absolute right-1 stroke-white"
                      onClick={() => {
                        removeImage(file);
                      }}
                    >
